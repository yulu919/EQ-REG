...
class Fconv_PCA(nn.Module):
    ...
    def __init__(self,  sizeP, inNum, outNum, tranNum=8, inP = None, padding=None, ifIni=0, bias=True, Smooth = True, iniScale = 1.0):
        ...
        self.inNum = inNum
        self.sizeP = sizeP
        Basisin = sio.loadmat('./model/Basis_Har.mat')
        tempB   = Basisin['Basis']
        Basis, Rank, weight = GetBasis_PCA(tempB, sizeP,tranNum,inP,Smooth)        
...
def GetBasis_PCA(tempB, sizeP,tranNum,inP,Smooth):
    BasisR = np.transpose(tempB, [0,1,3,2])

    BasisR = np.reshape(BasisR, [sizeP*sizeP*tranNum, 36])
