...
from model import common,F_Conv_YL
...
#         self.tau = nn.Parameter(self.taum, requires_grad=True)                      # for sparse rain map
#         self.f = nn.ReLU(inplace=True)
#         self.resm1 = MySequential(F_Conv_YL.MyConv(self.channels, self.channels, kernel_size=self.kernel_size, stride = 1, padding= (self.kernel_size-1)//2, dilation =1),
#                                   nn.BatchNorm2d(self.channels),
#                                   nn.ReLU(),
#                                   F_Conv_YL.MyConv(self.channels, self.channels, kernel_size=self.kernel_size, stride = 1, padding= (self.kernel_size-1)//2, dilation = 1),
#                                   nn.BatchNorm2d(self.channels),
#                                    )
#         self.resm2 = MySequential(F_Conv_YL.MyConv(self.channels, self.channels,kernel_size=self.kernel_size, stride = 1, padding= (self.kernel_size-1)//2, dilation = 1),
#                                   nn.BatchNorm2d(self.channels),
#                                   nn.ReLU(),
#                                   F_Conv_YL.MyConv(self.channels, self.channels, kernel_size=self.kernel_size, stride = 1, padding= (self.kernel_size-1)//2, dilation = 1),
#                                   nn.BatchNorm2d(self.channels),
#                                   )
#         self.resm3 = MySequential(F_Conv_YL.MyConv(self.channels, self.channels,kernel_size=self.kernel_size, stride = 1, padding= (self.kernel_size-1)//2, dilation = 1),
#                                   nn.BatchNorm2d(self.channels),
#                                   nn.ReLU(),
#                                   F_Conv_YL.MyConv(self.channels, self.channels, kernel_size=self.kernel_size, stride = 1, padding= (self.kernel_size-1)//2, dilation = 1),
#                                   nn.BatchNorm2d(self.channels),
#                                   )
#         self.resm4 = MySequential(F_Conv_YL.MyConv(self.channels, self.channels, kernel_size=self.kernel_size, stride = 1, padding= (self.kernel_size-1)//2, dilation = 1),
#                                   nn.BatchNorm2d(self.channels),
#                                   nn.ReLU(),
#                                   F_Conv_YL.MyConv(self.channels, self.channels, kernel_size=self.kernel_size, stride = 1, padding= (self.kernel_size-1)//2, dilation = 1),
#                                   nn.BatchNorm2d(self.channels),
...
class Xnet(MyModule):
    def __init__(self, channels):
        ...
        self.channels = channels
        self.kernel_size = 3
        self.resx1 = MySequential(F_Conv_YL.MyConv(self.channels, self.channels, kernel_size=self.kernel_size , stride = 1, padding= (self.kernel_size-1)//2,  dilation = 1),
                                  nn.BatchNorm2d(self.channels),
                                  nn.ReLU(),
                                  F_Conv_YL.MyConv(self.channels, self.channels, kernel_size=self.kernel_size , stride = 1, padding= (self.kernel_size-1)//2,  dilation = 1),
                                  nn.BatchNorm2d(self.channels),
                                  )
        self.resx2 = MySequential(F_Conv_YL.MyConv(self.channels, self.channels, kernel_size=self.kernel_size, stride = 1, padding= (self.kernel_size-1)//2, dilation = 1),
                                  nn.BatchNorm2d(self.channels),
                                  nn.ReLU(),
                                  F_Conv_YL.MyConv(self.channels, self.channels, kernel_size=self.kernel_size, stride = 1, padding= (self.kernel_size-1)//2, dilation = 1),
                                  nn.BatchNorm2d(self.channels),
                                  )
        self.resx3 = MySequential(F_Conv_YL.MyConv(self.channels, self.channels, kernel_size=self.kernel_size, stride = 1, padding= (self.kernel_size-1)//2, dilation = 1),
                                  nn.BatchNorm2d(self.channels),
                                  nn.ReLU(),
                                  F_Conv_YL.MyConv(self.channels, self.channels,  kernel_size=self.kernel_size, stride = 1, padding= (self.kernel_size-1)//2, dilation = 1),
                                  nn.BatchNorm2d(self.channels),
                                  )
        self.resx4 = MySequential(F_Conv_YL.MyConv(self.channels, self.channels, kernel_size=self.kernel_size, stride = 1, padding= (self.kernel_size-1)//2,  dilation = 1),
                                  nn.BatchNorm2d(self.channels),
                                  nn.ReLU(),
                                  F_Conv_YL.MyConv(self.channels, self.channels, kernel_size=self.kernel_size, stride = 1, padding= (self.kernel_size-1)//2,  dilation = 1),
                                  nn.BatchNorm2d(self.channels),
