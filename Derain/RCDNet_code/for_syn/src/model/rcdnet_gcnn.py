...
import numpy as np
from model import GCNN as fcnn
...
class Xnet(nn.Module):
    def __init__(self, channels):
        ...
        Smooth=False
        act=nn.ReLU(True)
        self.headx = nn.Sequential(fcnn.Fconv_PCA(kernel_size, channels, n_feats, tranNum, inP=kernel_size, padding=(kernel_size-1)//2, ifIni=1, Smooth=Smooth))
        m_body = [
            fcnn.ResBlock(
                fcnn.Fconv_PCA, n_feats, kernel_size, tranNum=tranNum, inP=inP, bn=False, act=act, res_scale=0.1, Smooth=Smooth
            ) for _ in range(4)
        ]
        ...
        self.out = nn.Sequential(fcnn.Fconv_PCA_out(kernel_size, n_feats, self.channels, tranNum, inP=kernel_size, padding=(kernel_size-1)//2, ifIni=0, Smooth=Smooth))
        # self.resx1 = nn.Sequential(nn.Conv2d(self.channels, self.channels, kernel_size=3, stride=1, padding=1, dilation=1))
    def forward(self, input):
