...
from model import common,F_Conv_YL_2tran
from model.My_sequential import MySequential
...
class Mainnet(MyModule):
    ...
    def forward(self, input):
        #  print(input.size()) # [32, 3, 64, 64]
        # save mid-updating results
         ListB = []
         ListCM = []

        # initialize B0 and Z0 (M0 =0)
         z0 = F.conv2d(input, self.w_z_f, stride =1, padding = 1)   #[32, 33, 64, 64]   # dual variable z with the channels self.num_Z

         input_ini = torch.cat((input, z0), dim=1) #[32, 36, 64, 64]
...
class Mnet(nn.Module):
    def __init__(self, channels):
        ...
        self.f = nn.ReLU(inplace=True)
        self.resm1 = nn.Sequential(nn.Conv2d(self.channels, self.channels, kernel_size=self.kernel_size, stride = 1, padding= (self.kernel_size-1)//2, dilation =1),
                                  nn.BatchNorm2d(self.channels),
                                  nn.ReLU(),
                                  nn.Conv2d(self.channels, self.channels, kernel_size=self.kernel_size, stride = 1, padding= (self.kernel_size-1)//2, dilation = 1),
                                  nn.BatchNorm2d(self.channels),
                                   )
        self.resm2 = nn.Sequential(nn.Conv2d(self.channels, self.channels,kernel_size=self.kernel_size, stride = 1, padding= (self.kernel_size-1)//2, dilation = 1),
                                  nn.BatchNorm2d(self.channels),
                                  nn.ReLU(),
                                  nn.Conv2d(self.channels, self.channels, kernel_size=self.kernel_size, stride = 1, padding= (self.kernel_size-1)//2, dilation = 1),
                                  nn.BatchNorm2d(self.channels),
                                  )
        self.resm3 = nn.Sequential(nn.Conv2d(self.channels, self.channels,kernel_size=self.kernel_size, stride = 1, padding= (self.kernel_size-1)//2, dilation = 1),
                                  nn.BatchNorm2d(self.channels),
                                  nn.ReLU(),
                                  nn.Conv2d(self.channels, self.channels, kernel_size=self.kernel_size, stride = 1, padding= (self.kernel_size-1)//2, dilation = 1),
                                  nn.BatchNorm2d(self.channels),
                                  )
        self.resm4 = nn.Sequential(nn.Conv2d(self.channels, self.channels, kernel_size=self.kernel_size, stride = 1, padding= (self.kernel_size-1)//2, dilation = 1),
                                  nn.BatchNorm2d(self.channels),
                                  nn.ReLU(),
                                  nn.Conv2d(self.channels, self.channels, kernel_size=self.kernel_size, stride = 1, padding= (self.kernel_size-1)//2, dilation = 1),
                                  nn.BatchNorm2d(self.channels),
...
#         self.tau = nn.Parameter(self.taum, requires_grad=True)                      # for sparse rain map
#         self.f = nn.ReLU(inplace=True)
#         self.resm1 = MySequential(F_Conv_YL_2tran.MyConv(self.channels, self.channels, kernel_size=self.kernel_size, stride = 1, padding= (self.kernel_size-1)//2, dilation =1, tran_num=2),
#                                   nn.BatchNorm2d(self.channels),
#                                   nn.ReLU(),
#                                   F_Conv_YL_2tran.MyConv(self.channels, self.channels, kernel_size=self.kernel_size, stride = 1, padding= (self.kernel_size-1)//2, dilation = 1, tran_num=2),
#                                   nn.BatchNorm2d(self.channels),
#                                    )
#         self.resm2 = MySequential(F_Conv_YL_2tran.MyConv(self.channels, self.channels,kernel_size=self.kernel_size, stride = 1, padding= (self.kernel_size-1)//2, dilation = 1, tran_num=2),
#                                   nn.BatchNorm2d(self.channels),
#                                   nn.ReLU(),
#                                   F_Conv_YL_2tran.MyConv(self.channels, self.channels, kernel_size=self.kernel_size, stride = 1, padding= (self.kernel_size-1)//2, dilation = 1, tran_num=2),
#                                   nn.BatchNorm2d(self.channels),
#                                   )
#         self.resm3 = MySequential(F_Conv_YL_2tran.MyConv(self.channels, self.channels,kernel_size=self.kernel_size, stride = 1, padding= (self.kernel_size-1)//2, dilation = 1, tran_num=2),
#                                   nn.BatchNorm2d(self.channels),
#                                   nn.ReLU(),
#                                   F_Conv_YL_2tran.MyConv(self.channels, self.channels, kernel_size=self.kernel_size, stride = 1, padding= (self.kernel_size-1)//2, dilation = 1, tran_num=2),
#                                   nn.BatchNorm2d(self.channels),
#                                   )
#         self.resm4 = MySequential(F_Conv_YL_2tran.MyConv(self.channels, self.channels, kernel_size=self.kernel_size, stride = 1, padding= (self.kernel_size-1)//2, dilation = 1, tran_num=2),
#                                   nn.BatchNorm2d(self.channels),
#                                   nn.ReLU(),
#                                   F_Conv_YL_2tran.MyConv(self.channels, self.channels, kernel_size=self.kernel_size, stride = 1, padding= (self.kernel_size-1)//2, dilation = 1, tran_num=2),
#                                   nn.BatchNorm2d(self.channels),
...
#         self.channels = channels
#         self.resx1 = nn.Sequential(nn.Conv2d(self.channels, self.channels, kernel_size=5, stride = 1, padding= 2,  dilation = 1),
#                                   nn.BatchNorm2d(self.channels),
#                                   nn.ReLU(),
#                                   nn.Conv2d(self.channels, self.channels, kernel_size=5, stride = 1, padding= 2,  dilation = 1),
#                                   nn.BatchNorm2d(self.channels),
#                                   )
#         self.resx2 = nn.Sequential(nn.Conv2d(self.channels, self.channels, kernel_size=5, stride = 1, padding= 2, dilation = 1),
#                                   nn.BatchNorm2d(self.channels),
#                                   nn.ReLU(),
#                                   nn.Conv2d(self.channels, self.channels, kernel_size=5, stride = 1, padding= 2, dilation = 1),
#                                   nn.BatchNorm2d(self.channels),
#                                   )
#         self.resx3 = nn.Sequential(nn.Conv2d(self.channels, self.channels, kernel_size=5, stride = 1, padding= 2, dilation = 1),
#                                  nn.BatchNorm2d(self.channels),
#                                   nn.ReLU(),
#                                   nn.Conv2d(self.channels, self.channels,  kernel_size=5, stride = 1, padding= 2, dilation = 1),
#                                   nn.BatchNorm2d(self.channels),
#                                   )
#         self.resx4 = nn.Sequential(nn.Conv2d(self.channels, self.channels, kernel_size=5, stride = 1, padding= 2,  dilation = 1),
#                                   nn.BatchNorm2d(self.channels),
#                                   nn.ReLU(),
#                                   nn.Conv2d(self.channels, self.channels, kernel_size=5, stride = 1, padding= 2,  dilation = 1),
#                                   nn.BatchNorm2d(self.channels),
...
class Xnet(MyModule):
    def __init__(self, channels):
        ...
        self.channels = channels
        self.kernel_size = 3
        self.resx1 = MySequential(F_Conv_YL_2tran.MyConv(self.channels, self.channels, kernel_size=self.kernel_size , stride = 1, padding= (self.kernel_size-1)//2,  dilation = 1, tran_num=2),
                                  nn.BatchNorm2d(self.channels),
                                  nn.ReLU(),
                                  F_Conv_YL_2tran.MyConv(self.channels, self.channels, kernel_size=self.kernel_size , stride = 1, padding= (self.kernel_size-1)//2,  dilation = 1, tran_num=2),
                                  nn.BatchNorm2d(self.channels),
                                  )
        self.resx2 = MySequential(F_Conv_YL_2tran.MyConv(self.channels, self.channels, kernel_size=self.kernel_size, stride = 1, padding= (self.kernel_size-1)//2, dilation = 1, tran_num=2),
                                  nn.BatchNorm2d(self.channels),
                                  nn.ReLU(),
                                  F_Conv_YL_2tran.MyConv(self.channels, self.channels, kernel_size=self.kernel_size, stride = 1, padding= (self.kernel_size-1)//2, dilation = 1, tran_num=2),
                                  nn.BatchNorm2d(self.channels),
                                  )
        self.resx3 = MySequential(F_Conv_YL_2tran.MyConv(self.channels, self.channels, kernel_size=self.kernel_size, stride = 1, padding= (self.kernel_size-1)//2, dilation = 1, tran_num=2),
                                  nn.BatchNorm2d(self.channels),
                                  nn.ReLU(),
                                  F_Conv_YL_2tran.MyConv(self.channels, self.channels,  kernel_size=self.kernel_size, stride = 1, padding= (self.kernel_size-1)//2, dilation = 1, tran_num=2),
                                  nn.BatchNorm2d(self.channels),
                                  )
        self.resx4 = MySequential(F_Conv_YL_2tran.MyConv(self.channels, self.channels, kernel_size=self.kernel_size, stride = 1, padding= (self.kernel_size-1)//2,  dilation = 1, tran_num=2),
                                  nn.BatchNorm2d(self.channels),
                                  nn.ReLU(),
                                  F_Conv_YL_2tran.MyConv(self.channels, self.channels, kernel_size=self.kernel_size, stride = 1, padding= (self.kernel_size-1)//2,  dilation = 1, tran_num=2),
                                  nn.BatchNorm2d(self.channels),
