...
parser.add_argument('--lamda', type=float, default=1e-8, help='loss weights')
opt = parser.parse_args()

log_name = 'log.txt'
log_path = './models/' + log_name
with open(log_path, 'a') as f:
    print("############### Begin Training ################", file=f, flush=True)
...
def train_model(net, optimizer, lr_scheduler, datasets):
    ...
    for epoch in range(opt.resume, opt.niter):
        ...
        for ii, data in enumerate(data_loader[phase]):
            ...
            Xma = torch.cat((Xma, Xma_rot), dim=0)
            GT = torch.cat((GT, GT_rot), dim=0)
            XLI = torch.cat((XLI, XLI_rot), dim=0)
            mask = torch.cat((mask, mask_rot), dim=0)

            net.train()
            optimizer.zero_grad()
            net.rot_num_in(rot_num=n)
            ...
            loss = opt.Xl2 * loss_l2X + opt.Xl1* loss_l1X + opt.Al1 * loss_l1A + opt.lamda * reg
            # back propagation
            loss.backward()
            optimizer.step()
            mse_iter = loss.item()
            ...
            if ii % 100 == 0:
                ...
                with open(log_path, 'a') as f:
                    print(template.format(epoch + 1, opt.niter, phase, ii, num_iter_epoch[phase], mse_iter, loss_l2X, loss_l1X, loss_l1A, lr), file=f, flush=True)
                    print(log_str.format(rmseu.item(),train_psnr.item(), train_ssim.item()), file=f, flush=True)
        ...
        print('-' * 100)
        with open(log_path, 'a') as f:
            print('{:s}: mse={:.3e}, PSNR={:4.2f}, ssim= {:5.4f}'.format(phase, mse_per_epoch[phase], psnr_per_epoch, ssim_per_epoch), file=f, flush=True)
        # adjust the learning rate
        lr_scheduler.step()
        # save model
        torch.save(net.state_dict(), os.path.join(opt.model_dir, 'OSCNet_latest.pt'))
        if (epoch+1) % 20 == 0 or (epoch+1)>180:
            # save model
            model_prefix = 'model_'
            ...
            torch.save(net.state_dict(), os.path.join(opt.model_dir, 'OSCNet_state_%d.pt' % (epoch+1)))
...
def fliter_reg(model):  # calculate filter regularization
    reg = 0
...
def rot(x,n):
    # angle = 2 * torch.pi / self.rot_num
    y = torch.rot90(x, n, [2, 3])
    return y

def rotateAny(images, n):
...
if __name__ == '__main__':
    ...
    netOSC = OSCNet(opt).cuda()
    print_network(netOSC)
    optimizerOSC = optim.Adam(netOSC.parameters(), betas=(0.5, 0.999), lr=opt.lr)
    schedulerOSC = optim.lr_scheduler.MultiStepLR(optimizerOSC, milestones=opt.milestone, gamma=0.5)
    ...
    if opt.resume:
        ...
        netOSC.load_state_dict(torch.load(os.path.join(opt.model_dir, 'OSCNet_state_' + str(opt.resume) + '.pt')))
    ...
    # train model
    train_model(netOSC, optimizerOSC, schedulerOSC, datasets)
