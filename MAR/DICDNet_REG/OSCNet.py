...
class OSCNet(MyModule):
    def __init__(self, args):
        ...
        ###############
        # para = {}
        # self.fcnn1 = fcnn.Fconv_PCA(sizeP=9, inNum=1, outNum=4, tranNum=8, inP=9, padding=(9-1)//2, ifIni=1, Smooth=True, iniScale=1.0)
        # print(self.fcnn1)
        # for name, parameter in self.fcnn1.named_parameters():
        #     print(name)
        #     para[name] = parameter
        # print("##shape", para['c'].size())
        # self.F_filter0 = FConv_filter()
        # self.F_filter = FConv_filter()
        #####################
        ...
        self.F_filter0 = nn.Parameter(Dic, requires_grad=True)
        self.F_filter = nn.Parameter(Dic, requires_grad=True)
        ...
        convert = torch.eye(self.Np+1, self.Np+1)
    ...
    def make_eta(self, iters, const):
        ...
        const_f = const_dimadd.expand(iters, -1) # size: (iters, 1)
    ...
    ## First using non-K version to initialize M and X, refer to Sec 1.2 in SM
        input = Xma #[16, 1, 64, 64]
        # print("input:" , input.size())
        # print("Xma:max{}min{}, XLI:max{}min{}".format(Xma.max(), Xma.min(), XLI.max(), XLI.min()))
        ...
        # for i in range(32):
        #     plt.subplot(4, 8, i+1)
        #     plt.imshow(F_filter[:, i, :, :].detach().cpu().squeeze(), cmap='gray')
        # plt.show()
        ...
        input_ini = torch.cat((XLI, P00), dim=1) # [16, 36, 64, 64]
        # print("input_ini:" , input_ini.size())
        XP_ini = self.proxNet_X_init(input_ini)
        X0 = XP_ini[:, :1, :, :]
        P0 = XP_ini[:, 1:, :, :]

        # updating M0--->M1
        ES = Mask*(input - X0) # 计算了金属影像中非金属区域的误差?
...
class Mnet(MyModule):
    ...
    def make_resblock(self, num_res):
        ...
        for i in range(num_res):
            layers.append(MySequential(Fc.MyConv(self.channels, self.channels, kernel_size=5, stride=1, padding=2, dilation=1),
                              nn.BatchNorm2d(self.channels),
                              nn.ReLU(),
                              Fc.MyConv(self.channels, self.channels, kernel_size=5, stride=1, padding=2, dilation=1),
                              nn.BatchNorm2d(self.channels),
                          ))
        return MySequential(*layers)
