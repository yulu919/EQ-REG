...
#             layers.append(MySequential(Fc.MyConv(self.channels, self.channels, kernel_size=self.ks, stride=1, padding=(self.ks-1)//2, dilation=1),
#                               nn.BatchNorm2d(self.channels),
#                               nn.ReLU(),
#                               Fc.MyConv(self.channels, self.channels, kernel_size=self.ks, stride=1, padding=(self.ks-1)//2, dilation=1),
#                               nn.BatchNorm2d(self.channels),
#                           ))
#         return MySequential(*layers)
...
class Mnet(MyModule):
    ...
    def make_resblock(self, num_res):
        layers = []
        for i in range(num_res):
            layers.append(MySequential(nn.Conv2d(self.channels, self.channels, kernel_size=self.ks, stride=1, padding=(self.ks-1)//2, dilation=1),
                              nn.BatchNorm2d(self.channels),
                              nn.ReLU(),
                              nn.Conv2d(self.channels, self.channels, kernel_size=self.ks, stride=1, padding=(self.ks-1)//2, dilation=1),
                              nn.BatchNorm2d(self.channels),
                          ))
        return MySequential(*layers)
    def forward(self, input):
